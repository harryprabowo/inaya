...
docker-db-migrate:
	...
	@sudo docker exec inaya-server yarn workspace server run sequelize db:migrate
	@echo "Migration on docker finished."

docker-db-undo-migration:
	@echo "Undoing all migration on docker..."
	@sudo docker exec inaya-server yarn workspace server run sequelize db:migrate:undo:all
	@echo "Undo all migration on docker finished."

docker-db-undo-latest:
	@echo "Undoing latest migration on docker..."
	@sudo docker exec inaya-server yarn workspace server run sequelize db:migrate:undo
	@echo "Undo latest migration on docker finished."

docker-db-seed-all:
	@echo "Running all seeds..."
	@sudo docker exec inaya-server yarn workspace server run sequelize db:seed:all
	@echo "Finished."